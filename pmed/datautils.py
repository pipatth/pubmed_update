...
def create_tables(conn):
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS article (id INTEGER PRIMARY KEY UNIQUE, title TEXT, abstract TEXT, pubyear INTEGER, jid INTEGER, keywords TEXT, pmc INTEGER, citations INTEGER) DEFAULT CHARSET=utf8mb4")
    conn.execute(
    ...
    conn.execute("CREATE INDEX article_cite_IDX USING BTREE ON article (citations);")
...
def update_cnt(conn, id, cnt):
    # update by id
    stmt = sqlalchemy.text('''UPDATE article SET citations = :c WHERE id = :i''')
